                ...
                <tbody>
                    @foreach ($aperturas as $item)
                        @php
                            ...
                            if ((!$item->status_curso AND !$item->status_solicitud) OR (!$item->status_curso AND ($item->status_solicitud=='VALIDADO' OR $item->status_solicitud=='RETORNO'))) {
                                $rojo = true;
                            }
                        @endphp
                        <tr @if($rojo)class='text-danger' @endif>
                            ...
                            <td class="text-center"> @if($item->status_curso) {{$item->status_curso}} @else @if ($item->status_solicitud) {{$item->status_solicitud}} @else {{"EN CAPTURA"}} @endif @endif </td>
                            ...
                            <td class="text-center">
                                @if ($item->munidad)
                                <a class="nav-link" href="{{route('solicitud.apertura.turnar', ['memo' => $item->munidad])}}">
                                    <i class="fa fa-search  fa-2x fa-lg text-info" title="Ver"></i>
                                </a>
                                @else
                                <a class="nav-link" href="{{route('solicitud.apertura.turnar', ['memo' => $item->num_revision])}}">
                                    <i class="fa fa-search  fa-2x fa-lg text-info" title="Ver"></i>
                                </a>
                                @endif
                            </td>
