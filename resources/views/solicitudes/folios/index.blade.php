            ...
            function editar(id,asignados) {
                $.ajax({
                    ...
                    success:  function (data) {
                        ...
                        let prefijo = '';
                        if (data.mod == 'GRAL') {
                            //Dividir letras con numeros
                            const folio = data.finicial ?? '';
                            const match = folio.match(/^([A-Za-z]+)(\d+)$/);
                            if (match) {
                                prefijo = match[1] ?? '';
                            } else {
                                console.warn('Folio sin formato esperado:', folio);
                            }
                        }
                        ...
                        if(data['num_inicio'])$("#finicial" ).val(prefijo + data['num_inicio']);
                        if(data['num_fin'])$("#ffinal" ).val(prefijo + data['num_fin']);
