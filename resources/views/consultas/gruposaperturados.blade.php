...
@section('content')   
    ...
    <div class="card-header">
        Consultas/ DTA- Grupos Aperturados
                    ...
                    <thead>
                        <tr>
                           <th scope="col" class="text-center" width="1%">#</th>
                            <th scope="col" class="text-center" width="8%">UNIDAD/AMÓVIL</th>                            
                            <th scope="col" class="text-center" >CLAVE</th>
                            <th scope="col" class="text-center" width="8%">ESCPECIALIDAD</th>
                            <th scope="col" class="text-center" width="12%">CURSO</th>                            
                            <th scope="col" class="text-center" width="8%">INSTRUCTOR</th>
                            <th scope="col" class="text-center" width="8%">CP</th>
                            <th scope="col" class="text-center" width="8%">HORAS</th>
                            <th scope="col" class="text-center" width="8%">COSTO/HORA</th>
                            <th scope="col" class="text-center" width="8%">IMPORTE</th>
                            <th scope="col" class="text-center" width="4%">CAPACITACIÓN</th>
                            ...
                            <th scope="col" class="text-center" width="5%">AGENDA</th>
                            <th scope="col" class="text-center" width="6%">ESTATUS</th>
                            <th scope="col" class="text-center" width="6%">VINCULADOR</th>
                        </tr>
                    </thead>
                    @if(isset($data))
                    <?php $i=1;   ?>
                    <tbody>
                        @foreach($data as $d)
                            <tr>
                                <td>{{ $i++ }}</td>
                                <td>{{ $d->unidad }}</td>
                                <td><div style="width: 100px;"> {{ $d->clave }}</div></td>
                                <td>{{ $d->espe }}</td>
                                <td>{{ $d->curso }}</td>                                
                                <td class="text-center" ><div style="width: 130px;">{{ $d->nombre }}</div></td>
                                <td class="text-center" >{{ $d->cp }}</td>
                                <td class="text-center" >{{ $d->dura }}</td>
                                <td class="text-center" >{{ number_format($d->costo, 2, '.', ',') }}</td>
                                <td class="text-center" >{{ number_format($d->importe, 2, '.', ',') }}</td>
                                ...
                                <td class="text-center" >{{ $d->cupo }}</td>
                                <td class="text-center" ><div style="width: 130px;">{!! $d->agenda !!}</div></td>
                                <td class="text-center" >{{ $d->status }}</td>
                                <td class="text-center" ><div style="width: 130px;">{{ $d->vinculador }}</div></td>
                            </tr>
                        @endforeach
                    </tbody>
     ...
     @section('script_content_js')
        ...
        <script language="javascript">
            $(document).ready(function(){
                $("#botonFILTRAR" ).click(function(){ $('#frm').attr('action', "{{route('consultas.gruposaperturados')}}"); $("#frm").attr("target", '_self'); $('#frm').submit(); });
                $("#botonXLS" ).click(function(){ $('#frm').attr('action', "{{route('consultas.gruposaperturados.xls')}}"); $("#frm").attr("target", '_blanck');$('#frm').submit();});
            });
            $(function() {
