        ...
        <div class="tab-content" id="nav-tabContent">
            ...
            @foreach ($data as $item)
                ...
                <div class="tab-pane fade <?php if($n==1) echo 'show active'; ?>" id="nav-{{$n}}" role="tabpanel" aria-labelledby="nav-{{$i}}-tab">                   
                    <form action="{{ url('/supervision/instructor/guardar') }}" name="frm{{$item->id}}" id="frm{{$item->id}}" method="post" enctype="multipart/form-data" class="was-validated"  novalidate>
                    @csrf        
                        ...
                        <table class="table">                      
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td>
                                  <div class="custom-control custom-checkbox mb-3">
                                    <input type="checkbox" name="ok_numero_apertura" value="1" class="custom-control-input" id="check{{$item->id}}[16]" @if($item->ok_numero_apertura) {{ 'checked'}} @endif required/>
                                    <label class="custom-control-label" for="check{{$item->id}}[16]"></label>
                                    <div class="invalid-feedback"> 
                                        <input type="text" name="obs_numero_apertua" value="{{$item->obs_numero_apertura}}" class="form-control" id="coment{{$item->id}}[16]" placeholder="Comentarios"  required/>
                                    </div>
                                  </div>
                              </td>
                            </tr>
