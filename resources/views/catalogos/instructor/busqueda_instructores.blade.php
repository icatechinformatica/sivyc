        ...
        <div class="row">
            <div class="col-lg-12 margin-tb">
                {!! html()->form('GET', route('instructor-inicio'))->class('form-inline')->open() !!}
                    ...
                    <button class="btn btn-sm btn-outline-primary py-2 my-sm-0 ml-3" type="submit">BUSCAR</button>
                    @can('instructor.create')
                        <a class="btn btn-sm btn-outline-success py-2 ml-3" href="{{route('instructor-vista-crear_new')}}"> Nuevo</a>
                    @endcan
                    @can('academico.exportar.instructores')
                        <a class="btn btn-sm btn-warning py-2 ml-3" href="{{route('academico.exportar.instructores')}}">CATÁLOGO XLS</a>
                        <a class="btn btn-sm btn-warning py-2 ml-3" href="{{route('academico.exportar.instructores.activos')}}">ACTIVOS XLS</a>
                    @endcan
                {!! html()->form()->close() !!}
            </div>
        </div>
                    ...
                    <th scope="row">{{$itemData->numero_control}}</th>
                        ...
                        <td class="text-center">
                            ...
                            @if ($itemData->status == 'Aprobado' || $itemData->status == 'BAJA')
                                    <a style="color: white;" class="btn btn-sm btn-outline-primary waves-effect waves-light py-2 mr-sm-4 " href="{{route('instructor-ver', ['id' => $itemData->id])}}">Mostrar</a>
                            @endif
                            @if ($itemData->status == 'VALIDADO' || $itemData->status == 'BAJA EN PREVALIDACION')
                                    <a style="color: white;" class="btn btn-sm btn-outline-primary waves-effect waves-light py-2 btn mr-sm-4 " href="{{route('instructor-ver', ['id' => $itemData->id])}}">Mostrar</a>
