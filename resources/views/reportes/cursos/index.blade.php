...
@section('content')
    ...
    <div class="card card-body" style=" min-height:450px;">
        ...
        @endif
        <br />
        {!! html()->form('POST', '#')->id('frm')->target('_blank')->attribute('accept-charset', 'UTF-8')->open() !!}
        <div class="container">
            <div class="form-row">
                <div class="form-group col-md-4">
                    {!! html()->text('clave')->id('clave')->class('form-control')->placeholder('CLAVE DEL CURSO')->attribute('aria-label', 'CLAVE DEL CURSO') !!}
                </div>
            </div>
    ...
    <script language="javascript">
        ...
        $(function() {
            $('#frm').validate({
                ...
                messages: {
                    clave: {
                        required: 'Por favor ingrese la clave del curso'
                    }
                }
            });
        });
        $(document).ready(function() {
            $("#botonASIST").click(function() {
                $('#frm').attr('action', "{{ route('reportes.asist.pdf') }}");
                $('#frm').submit();
            });
            $("#botonCALIF").click(function() {
                $('#frm').attr('action', "{{ route('reportes.calif.pdf') }}");
                $('#frm').submit();
            });
            $("#botonRIAC-INS").click(function() {
                $('#frm').attr('action', "{{ route('reportes.ins.pdf') }}");
                $('#frm').submit();
            });
            $("#botonRIAC-ACRED").click(function() {
                $('#frm').attr('action', "{{ route('reportes.acred.pdf') }}");
                $('#frm').submit();
            });
            $("#botonRIAC-CERT").click(function() {
                $('#frm').attr('action', "{{ route('reportes.cert.pdf') }}");
                $('#frm').submit();
            });
            $("#botonXLS-CONST").click(function() {
                $('#frm').attr('action', "{{ route('reportes.const.xls') }}");
                $('#frm').submit();
            });
        });
    </script>
