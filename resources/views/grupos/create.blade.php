                    ...
                    <div class="p-3 mb-2">
                        ...
                        <div class="row my-1">
                            <div class="form-group col-md-4 mb-1">
                                ...
                                {{ html()->select(
                                        'imparticion',
                                        [null => 'SELECCIONE EL TIPO DE IMPARTICIÓN'] + $tiposImparticion->pluck('imparticion', 'id')->toArray(),
                                    )->class('form-control')->required()->value($esNuevoRegistro ? null : $grupo->id_imparticion) }}
                            </div>
                            <div class="form-group col-md-4 mb-1">
                                {{ html()->label('MODALIDAD', 'modalidad')->class('form-label mb-1') }}
                                {{ html()->select('modalidad', [null => 'SELECCIONAR MODALIDAD'] + $modalidades->pluck('modalidad', 'id')->toArray())->class('form-control ')->required()->value($esNuevoRegistro ? null : $grupo->id_modalidad) }}
                            </div>
                            <div class="form-group col-md-4 mb-1">
                                {{ html()->label('UNIDAD/ACCIÓN MÓVIL', 'unidad_accion_movil')->class('form-label mb-1') }}
                                {{ html()->select('unidad_accion_movil', [null => 'SELECCIONAR'] + $unidades->pluck('unidad', 'id')->toArray())->class('form-control ')->required()->value($esNuevoRegistro ? null : $grupo->id_unidad) }}
                            </div>
                        </div>
                        <div class="row my-1">
                            <div class="form-group col-md-3 mb-1">
                                {{ html()->label('SERVICIO', 'servicio')->class('form-label mb-1') }}
                                {{ html()->select('servicio', [null => 'SELECCIONAR'] + $servicios->pluck('servicio', 'id')->toArray())->class('form-control ')->required()->value($esNuevoRegistro ? null : $grupo->id_servicio) }}
                            </div>
                            <div class="form-group col-md-9 mb-1">
                                {{ html()->label('CURSO', 'curso')->class('form-label mb-1') }}
                                {{ html()->select('curso', [null => 'SELECCIONA CURSO'] + $cursos->pluck('nombre_curso', 'id')->toArray())->class('form-control ')->required()->value($esNuevoRegistro ? null : $grupo->id_curso) }}
                            </div>
                        </div>
                        <div class="d-flex justify-content-end mt-3">
                            {{ html()->button('Guardar Info. General')->class(
                                    'btn btn-primary float-end
                                                                                                            rounded',
                                )->id('guardar_info_general')->type('button') }}
                        </div>
                        {{ html()->form()->close() }}
                    </div>
                </div>
                    ...
                    <div class="p-3 mb-2">
                        ...
                        <div class="row my-1">
                            <div class="form-group col-md-6 mb-1">
                                ...
                                {{ html()->select('municipio', [null => 'SELECCIONAR'] + $municipios->pluck('muni', 'id')->toArray())->class('form-control')->id('municipio-select')->required()->value($esNuevoRegistro ? null : $grupo->id_municipio) }}
                            </div>
                            <div class="form-group col-md-6 mb-1">
                                ...
                                @if ($esNuevoRegistro)
                                    {{ html()->select('localidad', [
                                            '' => 'SELECCIONAR MUNICIPIO
                                                                                                                                        PRIMERO',
                                        ])->class('form-control')->id('localidad-select')->disabled()->required() }}
                                @else
                                    {{ html()->select('localidad', ['' => 'SELECCIONAR MUNICIPIO'] + $localidades->pluck('localidad', 'id')->toArray())->class('form-control')->id('localidad-select')->disabled($esNuevoRegistro)->required()->value($esNuevoRegistro ? null : $grupo->id_localidad) }}
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-1">
                            ...
                            {{ html()->number('codigo_postal')->class(
                                    'form-control
                                                                                                            ',
                        ...
                        <div class="d-flex justify-content-end">
                            {{ html()->button('Guardar Ubicación')->class(
                                    'btn btn-primary float-end
                                                                                                            guardar-seccion',
                                )->id('guardar_ubicacion')->type('button') }}
                        </div>
                    </div>
                    {{ html()->form()->close() }}
                </div>
                    ...
                    <div class="p-3 mb-2">
                        ...
                        <div class="row mt-2">
                            <div class="form-group col-md-12 mb-2">
                                ...
                                {{ html()->select('organismo_publico', ['' => 'SELECCIONAR'] + $organismos_publicos->pluck('organismo', 'id')->toArray())->class(
                                        'form-control
                                                                                                                            ',
                                    )->required()->value($esNuevoRegistro ? null : $grupo->id_organismo_publico) }}
                            </div>
                            <div class="form-group col-md-12 mb-2">
                        ...
                        <div class="d-flex justify-content-end">
                            {{ html()->button('Guardar Organismo')->class(
                                    'btn btn-primary float-end
                                                                                                            guardar-seccion',
                                )->id('guardar_organismo') }}
                        </div>
                    </div>
                    {{ html()->form()->close() }}
                </div>
                            ...
                            <div class="col-md-12">
                                <div class="mb-2">
                                    ...
                                    {{ html()->select('medio_virtual', [
                                            ...
                                            2 => 'VIRTUAL
                                                                                                                                            2',
                                        ])->class('form-control ')->disabled(!$esNuevoRegistro && !$grupo->id_imparticion == 2)->value($esNuevoRegistro ? null : $grupo->id_imparticion) }}
                                </div>
                                <div class="mb-2">
                                    {{ html()->label('ENLACE VIRTUAL', 'enlace_virtual')->class('form-label mb-1') }}
                                    {{ html()->text('enlace_virtual')->class('form-control ')->disabled(!$esNuevoRegistro && !$grupo->id_imparticion == 2)->value($esNuevoRegistro ? null : $grupo->link_virtual) }}
                                </div>
                                <div class="mb-2">
                ...
                <div class="col-12 mb-4 step-section" id="agenda" style="display:none;">
                    ...
                    <div class="p-3 mb-2">
                        ...
                        <div class="d-flex justify-content-end mt-4">
                            {{ html()->button('Guardar Agenda')->class(
                                    'btn btn-primary float-end
                                                                                                            guardar-seccion',
                                )->id('guardar_agenda')->type('button') }}
                        </div>
                    </div>
                    {{ html()->form()->close() }}
                </div>
