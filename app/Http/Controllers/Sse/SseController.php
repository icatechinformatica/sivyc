...
class SseController extends Controller
{
    ...
    {
        // Desactivar buffers (importante para SSE en Windows/XAMPP)
        ...
        $response = new StreamedResponse(function () {
            ...
            $deadline  = time() + 90; // 90s por conexión
            $beatEvery = 10;          // keepalive cada N iteraciones
            $i = 0;
            ...
            while (true) {
                ...
                try {
                    // ⬇️ 2 consultas distintas para evitar (? IS NULL OR id > ?)
                    if ($lastId === null) {
                        $rows = DB::select("
                            SELECT
                              id,
                    ...
                    // Enviar el error al cliente como comentario SSE (no HTML)
                    $msg = str_replace(["\r", "\n"], ' ', $e->getMessage());
                    echo ": error {$msg}\n\n";
                    @ob_flush(); @flush();
                    usleep(800000);
