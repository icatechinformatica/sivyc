...
{   
    ...
    public function xls(Request $request){        
        ...
        $opcion = $request->opcion;
        if($unidad OR $fecha1 OR $fecha2){                     
            $data = DB::table('tbl_cursos as c')->where('clave','!=','0')->select('unidad','espe','clave','curso','mod','dura','inicio',
            'termino',DB::raw("CONCAT(hini,' A ',hfin) as horario"),'dia','horas',DB::raw("hombre+mujer as cupo"),'nombre',
            'cp','mujer','hombre','costo','tipo_curso','tipo','nota','muni','depen','munidad','mvalida','nmunidad','nmacademico','efisico','modinstructor','status','tcapacitacion');
            if($opcion == "TERMINADOS"){
                if($request->fecha1) $data = $data->where('c.termino','>=',$request->fecha1);
                if($request->fecha2) $data = $data->where('c.termino','<=',$request->fecha2);
            }else{
            ...
            $head = ['UNIDAD','ESPECIALIDAD','CLAVE','CURSO','MOD','DURA','INICIO','TERMINO','HORARIO','DIAS',	
            ...
            'MEMO DE AUTORIZACION DE REPROGRAMACION','ESPACIO','PAGO INSTRUCTOR','Estatus','CAPACITACION'];            
