            ...
            if($curso){
                ...
                ->select(
                    'i.matricula','i.alumno','i.sexo','i.escolaridad','i.discapacidad','i.abrinscri',
                        ...
                        DB::raw("(CASE WHEN i.calificacion!='NP' AND i.calificacion>'5' THEN 'X' END) as acreditado")
                )->where('i.id_curso',$curso->id)->where('i.status','INSCRITO')->orderby('i.alumno')->get();
            ...
            if($curso){
                ...
                $alumnos = DB::table('tbl_inscripcion as i')
                    ->select(
                        'i.matricula','i.alumno','i.sexo','i.escolaridad','i.discapacidad','i.abrinscri',
                            DB::raw("EXTRACT(year from (age('".$fecha_termino."',i.fecha_nacimiento))) as edad"),
                            DB::raw("(CASE WHEN i.calificacion!='NP' AND i.calificacion>'5' THEN 'X' END) as acreditado"),'f.folio',DB::raw("to_char(f.fecha_expedicion, 'DD/MM/YYYY') as fecha_expedicion")
