...
class BuzonController extends Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        return view('vistas_pat.buzon_pat', compact('data', 'mesGlob', 'mes'));
    }
    ...
    */
    public function pdforg_direc($mes_get, $opcion_get)
    {
        ...
        #VERIFICA SI SON OFICINAS CENTRALES O UNIDADES
        $val_opcionG = 2;
        if ($opcion_get == 'centrales') $val_opcionG = 0;
        else if($opcion_get == 'unidades') $val_opcionG = 1;
        else return;
        #Area Dpto
        $areas = Organismos::select('o.id', 'o.id_parent', 'o.nombre as nom_dpto', 'fun.nombre as funcionario', 'fun.cargo')
        ->join('tbl_funcionarios as fun', 'fun.id_org', 'o.id')
        ->from('tbl_organismos as o')
        ->where('o.activo', '=', 'true')
        ->where('dif_dpto', '=', $val_opcionG)
        ->orderBy('o.id', 'asc')
        ->get();
