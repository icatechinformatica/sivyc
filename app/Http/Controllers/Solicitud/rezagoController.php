...
class rezagoController extends Controller
{
    ...
    public function index(Request $request){
        ...
        elseif(session()->has('folio')) $valor = session('folio');
        session(['alumnos' => NULL]);
        if($valor){
            ...
            if(session('unidades')) $grupo = $grupo->whereIn('ar.unidad',session('unidades'));
            ...
            // var_dump($grupo);exit;
            if($grupo){
                session(['folio' => $grupo->folio_grupo]);
                $anio_hoy = date('y');
    ...
    public function store(Request $request){
        ...
        if(session('folio') AND session('grupo') AND session('alumnos')){
                    $grupo = session('grupo');   //var_dump($grupo);exit;
                    $alumnos = session('alumnos');   //var_dump($alumnos);exit;
