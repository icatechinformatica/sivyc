...
class grupoController extends Controller
{
    use catUnidades;
    use catApertura;
    // definir propiedades antes de la asignación en el controlador
    public $ejercicio;
    public $path;
    public $path_uploadFiles;
    public $path_files;
    public $path_files_cancelled;
    public $key;
    public $id_user;
    public $realizo;
    public $id_unidad;
        ...
        if (session()->has('folio_grupo')) {
            ...
            if (count($alumnos) > 0) {
                ...
                $requestData = $request->merge([
                    ...
                    'unidad' => $grupo->unidad,
                    'modalidad' => $mod,
                    'status_curso' => $grupo->status_curso,
                    'id_curso' => $grupo->id_curso
                ]);
                $cursos = $this->cmbcursos($requestData, true);
    ...
    public function cmbinstructor(Request $request){
        if (isset($request->id) and isset($request->inicio) and isset($request->termino)) {
            ...
            $instructores = $this->data_instructores($requestData);
...
//dd($grupo);
    ...
    public function cmbrepre(Request $request){
     ...
     private function validaCurso(Request $request){
        ...
        if(DB::table('alumnos_registro')->where('folio_grupo',$folio_grupo)->where('turnado','<>','VINCULACION')->exists())
            $message = "Grupo turnado al Académico.";
        if($message)return redirect()->route('preinscripcion.grupo')->with(['message' => $message]);
        if ($message) {
            return [
                'redirect' => redirect()->route('preinscripcion.grupo')->with('message', $message)
            ];
        }
    }
    ...
    private function validaAlumno(Request $request,$curp){
        ...
        $alumno = DB::table('alumnos_pre')
                    ...
                    DB::raw("CONCAT(apellido_paterno,' ',apellido_materno,' ',nombre) as alumno")
                    )
                    ->where('curp', $curp)->where('activo', true)->first();

        if(!$alumno->escolaridad) $message .= "En el módulo Preregistro de Aspirantes, ingrese la escolaridad del alumno.";
        if($alumno->edad < 15) $message .= "La edad del alumno no es valida, debe tener 15 años cumplidos al iniciar el curso.";
        $cursos_recibidos = DB::table(DB::raw("(select a.id_curso as curso from alumnos_registro as a
            ...
            ->first();
        if($cursos_recibidos->total >15 AND $alumno->curso_extra==false) $message .= "El alumno excede con el limite de cursos que puede tomar en el año.";
        ...
        if($seis_meses>=$request->inicio) $message .= "El alumno ya esta registrado en el curso o no ha cumplido 6 meses para volver a tomar el mismo curso.";
        if ($message) {
            $message .=  " ".$alumno->alumno.", CURP: " . $curp .".<br><br>";
            return [
                'message' => $message
            ];
        }
    ...
    //             $id_curso = $a_reg->id_curso ?? $request->id_curso;
    //             $horario = $a_reg->horario ?? $request->hini.' A '.$request->hfin;
    //             $inicio = $a_reg->inicio ?? $request->inicio;
    //             $termino = $a_reg->termino ?? $request->termino;
    //             $tipo = $a_reg->tipo_curso ?? $request->tipo;
    //             $id_cerss = $a_reg->id_cerss ?? $request->cerss;
    //             $id_muni = $a_reg->id_muni ?? $request->id_municipio;
    //             $clave_localidad = $a_reg->clave_localidad ?? $request->localidad;
    //             $organismo = $a_reg->organismo_publico ?? $request->dependencia;
    //             $id_organismo = $a_reg->id_organismo ?? DB::table('organismos_publicos')->where('organismo',$request->dependencia)->where('activo', true)->value('id');
    //             $grupo_vulnerable = $a_reg->grupo_vulnerable ?? DB::table('grupos_vulnerables')->where('id',$request->grupo_vulnerable)->value('grupo');
    //             $id_vulnerable = $a_reg->id_vulnerable ?? $request->grupo_vulnerable;
    //             $comprobante_pago = $a_reg->comprobante_pago ?? null;
    //             $modalidad = $a_reg->mod ?? $request->modalidad;
    //             $folio_pago = $a_reg->folio_pago ?? $request->folio_pago;
    //             $fecha_pago =  $a_reg->fecha_pago ?? $request->fecha_pago;
    //             $instructor = $a_reg->id_instructor ?? $request->instructor;
    //             $efisico = $a_reg->efisico ?? str_replace('ñ','Ñ',strtoupper($request->efisico));
    //             $medio_virtual = $a_reg->medio_virtual ?? $request->medio_virtual;
    //             $link_virtual = $a_reg->link_virtual ?? $request->link_virtual;
    //             $servicio = $a_reg->servicio ?? $request->tcurso;
    //             $cespecifico = $a_reg->cespecifico ?? $request->cespecifico;
    //             $fcespe = $a_reg->fcespe ?? $request->fcespe;
    //             $observaciones = $a_reg->observaciones ?? str_replace('ñ','Ñ',strtoupper($request->observaciones));
    //             if (($id_organismo == 358) OR ($modalidad=='EXT')) {
    //                 $depen_repre = $a_reg->depen_repre ?? $request->repre_depen;
    //                 $depen_telrepre = $a_reg->depen_telrepre ?? $request->repre_tel;
    //             } else {
    //                 $depen_repre = $a_reg->depen_repre ?? DB::table('organismos_publicos')->where('organismo',$request->dependencia)->where('activo', true)->value('nombre_titular');
    //                 $depen_telrepre = $a_reg->depen_telrepre ?? DB::table('organismos_publicos')->where('organismo',$request->dependencia)->where('activo', true)->value('telefono');
    //             }
    ...
    //                    'iduser_created' => $iduser_created, 'comprobante_pago' => $comprobante_pago,
    //                    'created_at' => date('Y-m-d H:i:s'), 'fecha' => date('Y-m-d'), 'id_cerss' => $id_cerss, 'cerrs' => $cerrs, 'mod' => $modalidad,
    //                    'grupo' => $folio_grupo, 'eliminado' => false, 'grupo_vulnerable' => $grupo_vulnerable, 'id_vulnerable' => $id_vulnerable,
    ...
    public function save(Request $request)
    {
        ...
        $folio_grupo = $request->folio_grupo ?? $this->genera_folio();

        // Validar curso
        $resultadoCurso = $this->validaCurso($request);
        if (is_array($resultadoCurso)) {
            return $resultadoCurso['redirect'] ?? redirect()->back();
        }

        // Obtener CURPs a procesar
        $patron = '/^[0-9A-Z]{2}-[0-9]{6}$/';
        if (preg_match($patron, $request->busqueda)) {
            if (!Gate::allows('alumnos-masivo')) {
                return redirect()->route('preinscripcion.grupo')
                    ->with(['message' => 'Operación no permitida!']);
            }
            $curps = DB::table('alumnos_registro')
                ->where('folio_grupo', $request->busqueda)
                ->pluck('curp')
                ->toArray();
        } else {
            $curps = [$request->busqueda];
        }

        // Si no hay CURPs, terminar
        if (empty($curps)) {
            return redirect()->route('preinscripcion.grupo')
                ->with(['message' => 'No se encontraron alumnos para procesar']);
        }

        // ===== OPTIMIZACIÓN: Cargar datos comunes UNA SOLA VEZ antes del ciclo =====

        // Cachear datos que no cambian por CURP
        $datosCache = [
            'id_especialidad' => DB::table('cursos')
                ->where('estado', true)
                ->where('id', $request->id_curso)
                ->value('id_especialidad'),

            'id_unidad' => DB::table('tbl_unidades')
                ->where('unidad', $request->unidad)
                ->value('id'),

            'organismo_data' => DB::table('organismos_publicos')
                ->where('organismo', $request->dependencia)
                ->where('activo', true)
                ->first(),

            'grupo_vulnerable' => DB::table('grupos_vulnerables')
                ->where('id', $request->grupo_vulnerable)
                ->value('grupo'),

            // Cache para consultas condicionales de organismo
            'organismo_358' => 358,
            'modalidad_ext' => 'EXT'
        ];

        // Cache para la consulta de matrículas SICE (para múltiples CURPs)
        $matriculasSiceCache = [];
        if (count($curps) > 1) {
            $matriculasSiceCache = DB::table('registro_alumnos_sice')
                ->where('eliminado', false)
                ->whereIn('curp', $curps)
                ->pluck('no_control', 'curp')
                ->toArray();
        }

        // Cache para alumnos_pre (actualización curso_extra)
        $alumnosPreActualizar = [];
        ...
        foreach ($curps as $curp) {
            ...
            if ($alumno && $folio_grupo) {
                // ===== OPTIMIZACIÓN: Obtener matrícula usando cache si es posible =====
                if (count($curps) > 1 && isset($matriculasSiceCache[$curp])) {
                    $matricula_sice = $matriculasSiceCache[$curp];
                } else {
                    $matricula_sice = DB::table('registro_alumnos_sice')
                        ->where('eliminado', false)
                        ->where('curp', $curp)
                        ->value('no_control');
                }
                ...
                if ($matricula_sice) {
                    $matricula = $matricula_sice;
                    // Acumular para actualización batch después
                    DB::table('registro_alumnos_sice')
                        ->where('curp', $curp)
                        ->update(['eliminado' => true]);
                } elseif (isset($alumno->matricula)) {
                    $matricula = $alumno->matricula;
                }

                // ===== OPTIMIZACIÓN: Usar datos cacheados en lugar de consultas repetitivas =====
                $id_especialidad = $a_reg->id_especialidad ?? $datosCache['id_especialidad'];
                $id_unidad = $a_reg->id_unidad ?? $datosCache['id_unidad'];
                $unidad = $a_reg->unidad ?? $request->unidad;
                $id_curso = $a_reg->id_curso ?? $request->id_curso;
                $horario = $a_reg->horario ?? $request->hini . ' A ' . $request->hfin;
                $inicio = $a_reg->inicio ?? $request->inicio;
                ...
                // Usar organismo_data cacheado
                $id_organismo = $a_reg->id_organismo ?? ($datosCache['organismo_data']->id ?? null);
                $grupo_vulnerable = $a_reg->grupo_vulnerable ?? $datosCache['grupo_vulnerable'];
                $id_vulnerable = $a_reg->id_vulnerable ?? $request->grupo_vulnerable;
                $comprobante_pago = $a_reg->comprobante_pago ?? null;
                $modalidad = $a_reg->mod ?? $request->modalidad;
                $folio_pago = $a_reg->folio_pago ?? $request->folio_pago;
                ...
                // ===== OPTIMIZACIÓN: Datos de representante con cache =====
                if (($id_organismo == $datosCache['organismo_358']) || ($modalidad == $datosCache['modalidad_ext'])) {
                    $depen_repre = $a_reg->depen_repre ?? $request->repre_depen;
                    $depen_telrepre = $a_reg->depen_telrepre ?? $request->repre_tel;
                } else {
                    $depen_repre = $a_reg->depen_repre ?? ($datosCache['organismo_data']->nombre_titular ?? null);
                    $depen_telrepre = $a_reg->depen_telrepre ?? ($datosCache['organismo_data']->telefono ?? null);
                }
                ...
                // ===== OPTIMIZACIÓN: Insert/Update (se mantiene igual) =====
                $result = DB::table('alumnos_registro')->updateOrInsert(
                    ['id_pre' => $alumno->id_pre, 'folio_grupo' => $folio_grupo],
                    [
                        'id_unidad' => $id_unidad,
                        ...
                        'efisico' => $efisico,
                        'medio_virtual' => $medio_virtual,
                        'link_virtual' => $link_virtual,
                        'servicio' => $servicio,
                        'cespecifico' => $cespecifico,
                        'fcespe' => $fcespe,
                        'observaciones' => $observaciones,
                        'depen_repre' => $depen_repre,
                        'depen_telrepre' => $depen_telrepre,
                        'requisitos' => $alumno->requisitos
                    ]
                );

                if ($result) {
                    session(['folio_grupo' => $folio_grupo]);
                    $message = "Operación Exitosa!!";

                    // Acumular para actualización batch de curso_extra
                    if ($alumno->curso_extra == true) {
                        $alumnosPreActualizar[] = $alumno->id_pre;
                    }
                } elseif ($message) {
                    return redirect()->route('preinscripcion.grupo')
        ...
        // ===== OPTIMIZACIÓN: Actualización batch de alumnos_pre (curso_extra) =====
        if (!empty($alumnosPreActualizar)) {
            DB::table('alumnos_pre')
                ->whereIn('id', $alumnosPreActualizar)
                ->where('curso_extra', true)
                ->update(['curso_extra' => false]);
        }
            ...
            if ($request->tcurso == "CERTIFICACION" and $horas == 10 or $request->tcurso == "CURSO") {
                if ($request->inicio <= $request->termino) {
                    ...
                    if ($mapertura AND $tc_curso->status_curso!='EDICION' AND
                        (DB::table('tbl_cursos')->where('mpreapertura',$mapertura)->where('folio_grupo','<>',$folio_grupo)->exists())) {
                        $message = 'Número de memorándum de apertura ocupado..';
                    } else {
                        // Optimización 1: Obtener datos del comprobante una sola vez
                        $file = $request->customFile;
                        $url_comprobante_data = DB::table('alumnos_registro')
                        ...
                        } elseif ($url_comprobante_data && $url_comprobante_data->comprobante_pago != null) {
                            $url_comprobante = $url_comprobante_data->comprobante_pago;
                        } else {
                        ...
                        // Optimización 2: Una sola consulta para obtener todos los datos del organismo
                        $organismo_data = DB::table('organismos_publicos')
                            ->where('organismo', $request->dependencia)
                            ->where('activo', true)
                            ->select('id', 'organismo', 'nombre_titular', 'telefono', 'sector')
                            ->first();

                        $id_organismo = $organismo_data ? $organismo_data->id : null;
                        $sector = $organismo_data ? $organismo_data->sector : null;

                        if (($id_organismo == 358) OR ($request->modalidad=='EXT')) {
                            $depen_repre = $request->repre_depen;
                            $depen_telrepre = $request->repre_tel;
                        } else {
                            $depen_repre = $organismo_data ? $organismo_data->nombre_titular : null;
                            $depen_telrepre = $organismo_data ? $organismo_data->telefono : null;
                        }
                        ...
                        $cerrs = $request->cerss ? true : NULL;

                        // Optimización 3: Obtener datos de unidad con una sola consulta
                        $unidad = DB::table('tbl_unidades')
                            ->select('id','cct', 'plantel','ubicacion')
                            ->where('unidad', $request->unidad)
                            ->first();

                        $id_ubicacion = DB::table('tbl_unidades')
                            ->where('unidad', $unidad->ubicacion)
                            ->value('id');

                        $municipio = DB::table('tbl_municipios')
                            ->select('id','muni','ze')
                            ->where('id', $request->id_municipio)
                            ->first();

                        $curso = DB::table('cursos as c')
                            ->select('c.id','c.nombre_curso','c.horas','c.rango_criterio_pago_maximo as cp','c.costo','e.nombre as espe',
                                'a.formacion_profesional as area','c.memo_validacion as mpaqueteria','e.clave as clave_especialidad')
                            ->join('especialidades as e','e.id','c.id_especialidad')
                            ->join('area as a','a.id','c.area')
                            ->where('c.id',$request->id_curso)
                            ->first();
                        ...
                        if ($instructor) {
                            /** CRITERIO DE PAGO */
                            $cp = ($instructor->cp > $curso->cp) ? $curso->cp : $instructor->cp;

                            /*CALCULANDO CICLO*/
                            $mes_dia1 = date("m-d", strtotime(date("Y-m-d")));
                            $mes_dia2 = date("m-d", strtotime(date("Y") . "-07-01"));
                            $ciclo = ($mes_dia1 >= $mes_dia2) ?
                                date("Y") . "-" . date("Y", strtotime(date("Y") . "+ 1 year")) :
                                date("Y", strtotime(date("Y") . "- 1 year")) . "-" . date("Y");
                            ...
                            if (!(DB::table('exoneraciones')
                                ...
                                // Preparar arrays para updates en lote
                                $alumno_updates = [];
                                $inscripcion_updates = [];
                            ...
                            $sx = DB::table('alumnos_registro')
                                ->select(
                                    DB::raw("COUNT(curp) as total"),
                                    DB::raw("SUM(CASE WHEN substring(curp,11,1) ='H' THEN 1 ELSE 0 END) as hombre"),
                            ...
                            if (!$ID) {
                                $ID = DB::table('tbl_cursos')
                                    ->where('unidad', $request->unidad)
                                    ->where('id', 'like', $PRE . '%')
                                    ->value(DB::raw('max(id)+1'));
                            }
                            if (!$ID) $ID = $PRE . '0001';
                            ...
                            $tipo_honorario = ($instructor->tipo_honorario == 'ASIMILADOS A SALARIOS') ?
                                'ASIMILADOS A SALARIOS' : 'HONORARIOS';
                            ...
                            if($tc_curso->status_curso=='EDICION'){
                                $result_curso = DB::table('tbl_cursos')
                                    ...
                                    ->update([
                                        ...
                                        'depen_representante' => $depen_repre,
                                        'depen_telrepre' => $depen_telrepre,
                                    ...
                                    $result_curso = DB::table('tbl_cursos')
                                        ...
                                        ->updateOrInsert(
                                            ...
                                            [
                                                ...
                                                'clave_especialidad' => $curso->clave_especialidad,
                                                'id_especialidad' => $id_especialidad,
                                                'instructor_escolaridad' => $instructor->escolaridad,
                                                'instructor_titulo' => $instructor->titulo,
                                                'instructor_sexo' => $instructor->sexo,
                            ...
                            if ($result_alumnos) {
                                if (($horario <> $alus->horario) OR
                                    ($request->id_curso <> $alus->id_curso) OR
                                    ($instructor->id <> $alus->id_instructor) OR
                                    ($request->inicio <> $alus->inicio) OR
                                    ($termino <> $alus->termino) OR
                                    ($id_especialidad <> $alus->id_especialidad)) {
    ...
    public function turnar(Request $request)
    {
        ...
        if (session('folio_grupo') and !$message) {

            if (DB::table('exoneraciones')->where('folio_grupo',session('folio_grupo'))->where('status','!=', 'CAPTURA')->where('status','!=','CANCELADO')->where('status','!=','AUTORIZADO')->exists()) {
                $message = "Solicitud de Exoneración o Reducción de couta en Proceso..";
                return redirect()->route('preinscripcion.grupo')->with(['message' => $message]);
            } else {
                $g = DB::table('tbl_cursos')->where('folio_grupo',session('folio_grupo'))->first();
                if ($g) {
                    if ($g->tipo!='PINS' AND ($g->mexoneracion=='NINGUNO' OR $g->mexoneracion==null OR $g->mexoneracion=='0') AND ($g->depen!='INSTITUTO DE CAPACITACION Y VINCULACION TECNOLOGICA DEL ESTADO DE CHIAPAS')) {
                        ...
                        $alumnos = DB::table('alumnos_registro')->where('folio_grupo',session('folio_grupo'))->get();
                        $comprobante = DB::table('alumnos_registro')->where('folio_grupo', session('folio_grupo'))->value('comprobante_pago');
                        $horas_agenda = DB::table('agenda')
                            ...
                            ->where('id_curso',session('folio_grupo'))->value('horas');
                        if ($horas_agenda == $g->dura) {
                            ...
                            //VOBO if($instructor_valido['valido']){
                                if($g->status_curso=="EDICION"){
                                    if($g->clave !='0') $result = DB::table('tbl_cursos')->where('folio_grupo', session('folio_grupo'))->where('clave','!=','0')->update(['status_curso' => 'AUTORIZADO']);
                                    else $result = DB::table('tbl_cursos')->where('folio_grupo', session('folio_grupo'))->where('clave','0')->update(['status_curso' => null]);
                                    ...
                                    $result = DB::table('alumnos_registro')->where('folio_grupo', session('folio_grupo'))->update(['turnado' => 'UNIDAD', 'fecha_turnado' => date('Y-m-d')]);
                                    if($result){
                                        DB::table('instructores')->where('id',$g->id_instructor)->where('curso_extra',true)->update(['curso_extra'=>false]);
                                        //DB::table('tbl_cursos')->where('folio_grupo',session('folio_grupo'))->whereNull('fpreapertura')->update(['fpreapertura'=>date('Y-m-d')]); HABILITAR PARA VOBO DG
                                        DB::table('tbl_cursos')->where('folio_grupo',session('folio_grupo'))->whereNull('fpreapertura')->update(['fpreapertura'=>date('Y-m-d'),'vb_dg'=>true]); //INHABILITAR PARA VOBO DG
    ...
    {
        $id = $request->id;
        if ($id and session('folio_grupo')) {
            if (DB::table('exoneraciones')->where('folio_grupo',session('folio_grupo'))->where('status','!=', 'CAPTURA')->where('status','!=','CANCELADO')->exists()) {
                $result = false;
            } else {
                //$result = DB::table('alumnos_registro')->where('folio_grupo', session('folio_grupo'))->where('id',$id)->update(['eliminado'=>true,'iduser_updated'=>$this->id_user]);
                if (count(DB::table('alumnos_registro')->where('folio_grupo',session('folio_grupo'))->where('eliminado',false)->get())>1) {
                    $result = DB::table('alumnos_registro')->where('folio_grupo', session('folio_grupo'))->where('id', $id)->delete();
                } else {
                    $result = false;
                }
            }
    ...
    public function subir_comprobante(Request $request)
    {
        ...
        $id = session('folio_grupo');
        if ($file and session('folio_grupo') == $request->folio_grupo) {
        ...
        if (session('folio_grupo') == $request->folio_grupo) {
            if ($request->busqueda1 AND $request->curpo) {
                $id = DB::table('alumnos_registro as ar')
                    ...
                    ->where('ar.folio_grupo',session('folio_grupo'))
